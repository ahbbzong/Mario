		...
		public override void LoadContent(ContentManager content)
		{
			SpriteDictionary = new Dictionary<Type, Dictionary<Type, Tuple<Texture2D,int,int>>>
			{
                ...
                {typeof(MiniBoss), new Dictionary<Type, Tuple<Texture2D,int,int>>{
                        ...
                        {typeof(LeftMovingMiniBossState), new Tuple<Texture2D, int, int>(content.Load<Texture2D>(SpriteString.MiniBossActive),1,4 )},
                        {typeof(RightMovingMiniBossState), new Tuple<Texture2D, int, int>(content.Load<Texture2D>(SpriteString.MiniBossActive),1,4 )}
